...
**A recorded run SHOULD have an ELAN project sitting beside it** — generated by
`scripts/eaf_make.py` with the video AND the game-only audio both linked, and
two empty tiers: **faults** and **scene identity**. Look for the `.eaf` next to
the **`.mp4`** in the evidence folder for that date (it is named after the
video, not after the run log).

> **BUT DO NOT ASSUME IT IS THERE (A741, 2026-08-30).** This paragraph used to
> promise it categorically. **The wiring that produces it has never actually
> run** — it was committed 69 minutes AFTER the only observed run since, and no
> run has happened since the commit. **If there is no `.eaf`, that is a real
> failure and worth saying so**; it is not you looking in the wrong place. The
> run's own stanza in `docs/observed-runs.md` records what was produced on an
> `annotate:` line — if that line is missing entirely, the wiring was not
> reached.
