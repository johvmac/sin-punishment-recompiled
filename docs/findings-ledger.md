...
| T15 | MEASURED | **A26 does not block the stack-drift line (A68).** The generated C in `RecompiledFuncs/` annotates **every** emitted line with its original instruction address (`// 0x800BA1A4: addiu $sp, $sp, -0x18`), including main-segment functions. The next step needs only each callee's **epilogue instruction address** for a `before_vram` hook, and that is directly greppable today. Splat/m2c (A26) buys *readable pseudocode*, which is a comprehension aid, not a prerequisite. Scope: checked in `RecompiledFuncs/funcs_20.c` for `main_func_800BA1A4`, which is inside T10's blind range | 2026-08-18 |
