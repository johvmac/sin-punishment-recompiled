...
| A64 | MEASURED | **NOT a steady leak — it is stable then bursts.** Entry `sp` is a constant `0x8007B9F8` at frames 200, 400, 600 **and 800**, then `0x8007B920` by frame 1000 — a drop of `0xD8`, i.e. **9 x 0x18 within those 200 frames**, concentrated in a burst around frame ~990 (consecutive per-call drops were seen at n=992, 993, 994...). **Everything balances perfectly for the first ~990 frames** | 2026-08-18; `[cpf]` + `[cbA1A4]` |
...
| A65 | OPEN | **The question is now "what happens at frame ~990", not "what leaks each frame".** Since the chain balances for 990 frames, the defect is triggered by a state change, not present from boot. Cheapest next: bisect *within* the burst — log entry `sp` plus the game's scene/state globals every frame across frames 950-1050 |  |
