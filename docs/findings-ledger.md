...
| A56 | OPEN | **Test vtable bounds properly (A55 is withdrawn, but the idea survives).** The right measurement is the **display-list entry** type at the `D_80059748` dispatch — not the node type. Log it and flag any value >= 18. If out-of-range values occur, the walker calls a bogus pointer, which would explain the leak, the run-varying onset (A48), and why no generated function is missing a restore (A51) — because the leak would not be in a recompiled function at all. Also bound `D_8006FC90` while there |  |
